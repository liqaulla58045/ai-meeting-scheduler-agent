...
def main():
    st.set_page_config(page_title="AI Meeting Scheduler Agent", page_icon="📅", layout="wide")
    st.title("📅 AI Meeting Scheduler Agent")
    st.write("Natural-language based meeting scheduler powered by AI.")

    with st.sidebar:
        st.header("⚙️ Settings")
        use_google = st.checkbox("Simulate Google Calendar integration", value=False)
        st.markdown("---")
        st.write("Made for Rooman AI Internship Challenge")

    tab1, tab2 = st.tabs(["➕ Schedule Meeting", "📋 View Meetings"])

    with tab1:
        st.subheader("Schedule via Natural Language")
        st.caption("Example: *Schedule a 30-minute sync with Rahul tomorrow at 3 PM about project status*")
        ...
        if st.button("🔍 Understand & Propose Meeting"):
            if not user_input.strip():
                ...
                st.write(f"**Start:** {parsed['start_time'].strftime('%Y-%m-%d %H:%M')}")
                st.write(f"**End:** {parsed['end_time'].strftime('%Y-%m-%d %H:%M')}")

                confirm = st.button("✅ Confirm & Schedule")
        ...
        if not meetings:
            ...
            for m in meetings:
                with st.container():
                    ...
                    st.caption(f"Created at: {m.created_at.strftime('%Y-%m-%d %H:%M')}")
